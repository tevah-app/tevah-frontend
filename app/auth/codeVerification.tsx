...
//     if (text.length > 1 && text.length <= 6 && /^\d+$/.test(text)) {
...
//     if (key === 'Backspace' && index === 5 && codeArray[5] === '') {
//       setCodeArray(['', '', '', '', '', '']);
//     }
//   };
...
//       ) {
//         await signUpAttempt.update({ password });
...
//       >
//         {showSuccess && <SuccessfulCard />}
//         {showFailure && <FailureCard onClose={hideDropdown} />}
//       </RNAnimated.View>
...
//             Enter the 6-digit code we sent to you at {emailAddress}
//           </Text>
//         </View>
...
//     try {
//       const response = await fetch("http://192.168.100.47:4000/api/auth/verify-email", {
//         method: "POST",
...
//     setIsResending(true);
//     try {
//       const response = await fetch("http://192.168.100.47:4000/api/auth/resend-code", {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
...
export default function CodeVerification() {
  ...
  const onVerifyPress = async () => {
    ...
    try {
      const response = await fetch("http://192.168.43.62:4000/api/auth/verify-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: emailAddress,
